...
export default function SupportChatPage() {
    ...
    useEffect(() => {
        async function initChat() {
            ...
            if (settings?.value) {
                // Handle both boolean and string 'true' cases safely
    ...
    async function handleSendMessage(e: React.FormEvent) {
        ...
        try {
            ...
            setMessages(prev => [...prev, optimisticMsg]) // Add immediately
            ...
            if (isSupportOnline) {
                ...
                const { data: ticket } = await supabase
                    ...
                    .select('id') // Just to check success
                    .single()
            ...
            // Remove optimistic message if needed, but Realtime will handle it, strictly we should dedupe if using realtime.
            // For simplicity here, relying on realtime to push the actual saved message which might duplicate if we don't be careful.
            // Actually, best practice: Don't optimistic update if relying on Realtime subscription to same table to avoid flicker/dupes, 
            // OR filter out local optimistic ones when real one arrives.
            // Let's REMOVE the optimistic update for stability since we have realtime listener.
        ...
        <div className="flex flex-col h-[calc(100vh-120px)] bg-white dark:bg-slate-900 rounded-xl border shadow-sm max-w-4xl mx-auto overflow-hidden">
            ...
            <div className="p-4 border-b flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="font-semibold text-lg">Support Chat</h1>
